  ...
  class Parser
    module Visitors
      class Citrus
        ...
        def visit_named(_name, parslet)
          parslet.accept(self)
        end

        def visit_sequence(parslets)
          elements = parslets.map { |el| el.accept(self) }.join(' ')
          "(#{elements})"
        end

        def visit_repetition(_tag, min, max, parslet)
          foo = parslet.accept(self)
          "#{foo}#{min}*#{max}"
        end

        def visit_alternative(alternatives)
          elements = alternatives.map { |el| el.accept(self) }.join(' | ')
          "(#{elements})"
        end

        def visit_lookahead(positive, bound_parslet)
          foo = bound_parslet.accept(self)
          return ('&' + foo) if positive
