...
module Parslet
  class Pattern
    ...
    #
    class SubtreeBind < Struct.new(:symbol)
      def variable_name
        symbol
      end
      ...
      def bind_type_name
        md = self.class.name.match(/(\w+)Bind/)
        return md.captures.first.downcase if md
    ...
    class SimpleBind < Parslet::Pattern::SubtreeBind
      def can_bind?(subtree)
        ![Hash, Array].include?(subtree.class)
      end
    end

    # Binds a symbol to a sequence of simple leafs ([element1, element2, ...])
    class SequenceBind < Parslet::Pattern::SubtreeBind
      def can_bind?(subtree)
        subtree.is_a?(Array) &&
          (subtree.none? { |el| [Hash, Array].include?(el.class) })
      end
    end
  end
end
